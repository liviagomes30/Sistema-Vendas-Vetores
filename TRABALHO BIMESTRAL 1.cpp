...
void LimparSaida(void){
	int x1 = 37, x2 = 77, y1 = 7, y2 = 20, i, j;
	for(i=x1; i<x2; i++)
		for(j=y1; j<y2; j++){
			gotoxy(i,j);
			printf(" ");
...
void LimparMsg(void){
	int x1 = 15, x2 = 76, y1 = 23, y2 = 25, i, j;
	for(i=x1; i<x2; i++)
		for(j=y1; j<y2; j++){
			gotoxy(i,j);
			printf("  ");
...
void InserirDados(TpClientes Cliente[], TpFornecedores Fornecedor[], TpProdutos Produto[], int &TLC, int &TLF, int &TL)
{
